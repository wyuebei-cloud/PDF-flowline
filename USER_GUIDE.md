...
7. **Flat segments**: When two adjacent elevations are equal there is no flow direction — the segment is drawn as a plain line (no arrowhead) labeled **FLAT**.
...
7. **平坡段**：相邻两点高程相等时没有流向——该段只画连线不画箭头，标签显示 **FLAT**。
